...
public class Get_Sound_New extends javax.swing.JPanel {
    ...
    public void set(int ID, byte[] sound, String time) {
        this.sound = sound;
        if (MethodC.getFriends().get(ID).getImage() != null) {
            profile.setIcon(MethodC.getFriends().get(ID).getImage());
        }
        lbName.setText(MethodC.getFriends().get(ID).getfName());
    ...
    private void playMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_playMouseReleased
        if (over) {
            if (!p) {
                play.setValue(0);
                MethodC.getRecoder().playAudio(sound, play);
                p = true;
            } else {
                MethodC.getRecoder().stopPlay();
                p = false;
            }
        }
